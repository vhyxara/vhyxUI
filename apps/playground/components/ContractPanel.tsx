...
const FIELD_DESCRIPTIONS: Record<string, string> = {
  ...
  intent: 'What the component does when activated — used by agents to understand purpose.',
  safetyLevel: 'low = safe, medium = review recommended, high = requires confirmation.',
  destructive: 'true if this action deletes or permanently modifies data.',
  requiresConfirmation: 'true if an additional user confirmation is needed before executing.',
...
export function ContractPanel({ def, props }: ContractPanelProps): React.ReactElement {
  ...
  const isHighlighted = (key: string): boolean => {
    if (def.id === 'button' && props['variant'] === 'destructive') {
      return key === 'safetyLevel' || key === 'destructive' || key === 'requiresConfirmation';
    }
    return false;
  };
  ...
  return (
    <div className="pg-contract-panel">
      <p className="pg-panel-hint">
        Default VhyxSeal contract — merged with your <code>contract</code> prop at runtime.
      </p>
      {def.id === 'button' && props['variant'] === 'destructive' && (
        <div className="pg-contract-upgrade-notice">
          <span className="pg-contract-upgrade-dot" />
          <span>Destructive variant auto-upgrades contract safetyLevel, destructive, and requiresConfirmation.</span>
        </div>
      )}
      <div className="pg-contract-table">
        {Object.entries(contract).map(([key, value]) => {
          ...
          return (
            <div key={key} className="pg-contract-row" data-highlighted={highlighted ? 'true' : 'false'} title={desc}>
      ...
      <a
        href="https://docs.vhyxui.dev/agent-contracts"
